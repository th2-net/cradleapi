...
public abstract class KeyspaceCreator {
	...
	public void createKeyspace() throws IOException, CradleStorageException	{
		if (obtainKeyspaceMetadata() != null)
			throw new CradleStorageException("Keyspace '" + keyspace + "' already exists");
		
		logger.info("Creating keyspace '{}'", keyspace);
		CreateKeyspaceStart createKeyspaceStart = SchemaBuilder.createKeyspace(keyspace);
		CreateKeyspace createKeyspace = settings.getNetworkTopologyStrategy() != null
				? createKeyspaceStart.withNetworkTopologyStrategy(settings.getNetworkTopologyStrategy().asMap())
				: createKeyspaceStart.withSimpleStrategy(settings.getKeyspaceReplicationFactor());
		queryExecutor.executeQuery(createKeyspace.asCql(), true);
	...
	private void awaitKeyspaceReady() throws CradleStorageException	{
		...
		while (obtainKeyspaceMetadata() == null && timeRemaining > 0) {
			attempt++;
	...
	protected KeyspaceMetadata getKeyspaceMetadata() {
		...
		obtainKeyspaceMetadata();
		return keyspaceMetadata;
	}


	private KeyspaceMetadata obtainKeyspaceMetadata()	{
		CqlSession session = queryExecutor.getSession();
		session.refreshSchema();
		keyspaceMetadata = session.getMetadata().getKeyspace(keyspace).orElse(null);
		return keyspaceMetadata;
	}
