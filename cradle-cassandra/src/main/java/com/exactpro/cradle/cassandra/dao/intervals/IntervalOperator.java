...
public interface IntervalOperator {
    ...
    @Query("UPDATE ${qualifiedTableId} SET "+INTERVAL_LAST_UPDATE_TIME+"=:lastUpdateTime, "+INTERVAL_LAST_UPDATE_DATE+"=:lastUpdateDate WHERE "+INSTANCE_ID+"=:instanceId AND "+ INTERVAL_START_DATE +"=:intervalStartDate AND "+INTERVAL_START_TIME+"=:intervalStartTime IF "+INTERVAL_LAST_UPDATE_TIME+"=:previousLastUpdateTime AND "+INTERVAL_LAST_UPDATE_DATE+"=:previousLastUpdateDate")
    CompletableFuture<AsyncResultSet> setIntervalLastUpdateTimeAndDate(UUID instanceId, LocalDate intervalStartDate, LocalTime intervalStartTime, LocalTime lastUpdateTime, LocalDate lastUpdateDate, LocalTime previousLastUpdateTime, LocalDate previousLastUpdateDate, Function<BoundStatementBuilder, BoundStatementBuilder> attributes);

    @Query("UPDATE ${qualifiedTableId} SET "+RECOVERY_STATE_JSON+"=:recoveryStateJson WHERE "+INSTANCE_ID+"=:instanceId AND "+ INTERVAL_START_DATE +"=:intervalDate AND "+INTERVAL_START_TIME+"=:intervalStartTime IF "+RECOVERY_STATE_JSON+"=:previousRecoveryStateJson")
    CompletableFuture<AsyncResultSet> updateRecoveryState(UUID instanceId, LocalDate intervalDate, LocalTime intervalStartTime, String recoveryStateJson, String previousRecoveryStateJson, Function<BoundStatementBuilder, BoundStatementBuilder> attributes);

    @Query("UPDATE ${qualifiedTableId} SET "+INTERVAL_PROCESSED+"=:processed WHERE "+INSTANCE_ID+"=:instanceId AND "+ INTERVAL_START_DATE +"=:intervalDate AND "+INTERVAL_START_TIME+"=:intervalStartTime IF "+INTERVAL_PROCESSED+"=:previousProcessed")
    CompletableFuture<AsyncResultSet> setIntervalProcessed(UUID instanceId, LocalDate intervalDate, LocalTime intervalStartTime, boolean processed, boolean previousProcessed, Function<BoundStatementBuilder, BoundStatementBuilder> attributes);
}
