...
public class TestEventUtilsBenchmark {
    ...
    public static class TestEventState {
        public static final BookId BOOK_ID = new BookId("Book1");
        public static final String SCOPE = "Scope1";
        public static final int MAX_SIZE = 1_024 * 1_024;
        public static final long THRESHOLD = new CoreStorageSettings().calculateStoreActionRejectionThreshold();

        private StoredTestEventId parentId;
        private StoredTestEventId eventId;
        ...
        public void init() throws CradleStorageException {
            parentId = new StoredTestEventId(BOOK_ID, SCOPE, Instant.now(), UUID.randomUUID().toString());
            batchId = new StoredTestEventId(BOOK_ID, SCOPE, Instant.now(), UUID.randomUUID().toString());
            eventId = new StoredTestEventId(BOOK_ID, SCOPE, Instant.now(), UUID.randomUUID().toString());
            ...
            event = new TestEventSingleToStoreBuilder(THRESHOLD)
                    ...
                    .content(RandomStringUtils.randomAlphabetic(600_000).getBytes())
                    .build();

            batch.addTestEvent(event);

            serializedEvent = TestEventUtils.serializeTestEvent(event).getSerializedData();
